...
/**
  ...
  *          using the STM32C0xx TIM LL API.
...
int main(void)
{
  ...
  /* clock frequency is equal to the timer frequency.                        */
    /* In this example TIM1 input clock (TIM1CLK) frequency is set to APB clock*/
  /*  (PCLK), since APB pre-scaler is equal to 1.                            */
  /*    TIM1CLK = TPCLK                                                       */
  /*    TPCLK = HCLK                                                          */
  ...
  LL_DMA_ConfigAddresses(DMA1, LL_DMA_CHANNEL_1, (uint32_t)&aCCValue, (uint32_t)&TIM1->CCR1, LL_DMA_GetDataTransferDirection(DMA1, LL_DMA_CHANNEL_1));
