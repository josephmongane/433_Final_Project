...
int main(void)
{
 ...
 /* Clear OPTVERR bit set on virgin samples */
  ...
  MemoryProgramStatus = Check_Program(FLASH_USER_START_ADDR, FLASH_USER_END_ADDR, DATA_32);

  /*Check if there is an issue to program data*/
  if(MemoryProgramStatus != 0)
  {
    /* Make LED1 blink with 1s period to indicate an error after programming */
    BSP_LED_On(LED1);
    HAL_Delay(1000);
    BSP_LED_Off(LED1);
    HAL_Delay(1000);
...
void Error_Handler(void)
{
  ...
  while(1)
  {
    /* Make LED1 blink (100ms on, 2s off) when there is an issue during erase or program procedure */
    BSP_LED_On(LED1);
    HAL_Delay(100);
    BSP_LED_Off(LED1);
    HAL_Delay(2000);
