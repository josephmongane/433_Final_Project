...
static VOID Led_Toggle(GPIO_TypeDef *GPIOx, uint16_t GPIO_Pin, UINT iter, GPIO_PinState PinState);
...
void ThreadOne_Entry(ULONG thread_input)
{
  ...
  while(1)
  {
    ...
    if (APP_SYNC_GET(&SyncObject, TX_NO_WAIT) == TX_SUCCESS)
    {
      ...
      Led_Toggle(LED1_GPIO_Port, LED1_Pin, 10,GPIO_PIN_RESET);
...
void ThreadTwo_Entry(ULONG thread_input)
{
  ...
  while(1)
  {
    ...
    if (APP_SYNC_GET(&SyncObject, TX_NO_WAIT) == TX_SUCCESS)
    {
      ...
      Led_Toggle(LED2_GPIO_Port, LED2_Pin, 10,GPIO_PIN_SET);
      /*release the sync object*/
      APP_SYNC_PUT(&SyncObject);
