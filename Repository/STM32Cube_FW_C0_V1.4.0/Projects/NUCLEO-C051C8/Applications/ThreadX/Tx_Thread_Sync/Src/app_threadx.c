...
void ThreadOne_Entry(ULONG thread_input)
{
  ...
  while(1)
  {
    ...
    if (APP_SYNC_GET(&SyncObject, TX_NO_WAIT) == TX_SUCCESS)
    {
      ...
      Led_Toggle(LED_GREEN_GPIO_Port, LED_GREEN_Pin, 10,50);
...
void ThreadTwo_Entry(ULONG thread_input)
{
  ...
  while(1)
  {
    ...
    if (APP_SYNC_GET(&SyncObject, TX_NO_WAIT) == TX_SUCCESS)
    {
      ...
      /*Sync object acquired toggle the LED_BLUE each 200ms for 5s*/
      Led_Toggle(LED_GREEN_GPIO_Port, LED_GREEN_Pin, 25,20);
      /*release the sync object*/
      APP_SYNC_PUT(&SyncObject);
