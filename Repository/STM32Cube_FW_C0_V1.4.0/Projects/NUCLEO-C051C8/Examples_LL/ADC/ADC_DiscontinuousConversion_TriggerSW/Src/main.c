...
/**
  ******************************************************************************
  * @file    Examples_LL/ADC/ADC_DiscontinuousConversion_TriggerSW/Src/main.c
...
int main(void)
{
  ...
  while (1)
  {
    for (tmp_index = 0; tmp_index < ADC_CONVERTED_DATA_BUFFER_SIZE; tmp_index++)
    {
      ...
      uhADCxConvertedData[tmp_index] = LL_ADC_REG_ReadConversionData32(ADC1);
...
static void MX_ADC1_Init(void)
{
  ...
  */
  ...
  LL_ADC_REG_SetSequencerChAdd(ADC1, LL_ADC_CHANNEL_4|LL_ADC_CHANNEL_VREFINT);
