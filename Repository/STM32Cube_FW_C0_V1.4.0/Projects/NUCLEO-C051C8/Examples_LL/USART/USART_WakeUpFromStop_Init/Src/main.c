...
/**
  ...
  */
uint8_t aTextInfo[] = "\r\nUSART1 Example : MCU will now enter in Stop mode.\n\rEnter any character for waking up MCU.\r\n";
...
void SystemClock_Config(void)
{
  ...
  LL_RCC_SetHSIKERDiv(LL_RCC_HSIKER_DIV_3);
  /* Set AHB prescaler*/
...
static void MX_USART1_UART_Init(void)
{
  ...
  LL_RCC_SetUSARTClockSource(LL_RCC_USART1_CLKSOURCE_HSIKER);
...
/**
  * @brief  Function to configure USART1 for being ready to enter Stop mode.
  * @param  None
  * @retval None
  */
void PrepareUSARTToStopMode(void)
